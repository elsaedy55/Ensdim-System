...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    builtIntro_en: 'We worked with Triboo across two stages: strategic product thinking, then MVP execution.',
    builtIntro_ar: 'عملنا مع فريق ترايبوو على مرحلتين: مرحلة التفكير والبناء الاستراتيجي، ثم مرحلة تنفيذ المنتج الأولي.',
    builtSections: [
      {
  ...
  },
  {
    ...
    situation_en: "The company used a traditional way of receiving clients and following up on quotation requests. The team relied heavily on WhatsApp, calls, Excel files, and manual distribution of client numbers to sales representatives. As campaigns increased and the company aimed to scale, the problem became clearer: every client had a different status, every request needed follow-up, and every project had many details that needed to be explained before a client could move forward. This made follow-up exhausting, reduced visibility for management, and made it harder to know the status of each client or request at the right time. The company wanted to expand and reach a wider audience, but the old workflow meant that every increase in demand required more sales hiring just to handle replies, explanations, follow-up, and data entry.",
    situation_ar: 'كانت الشركة تعمل بالطريقة التقليدية في استقبال العملاء ومتابعة طلبات الأسعار. الاعتماد كان كبيرًا على الواتساب، المكالمات، ملفات الإكسيل، وتوزيع أرقام العملاء يدويًا على فريق المبيعات. مع زيادة الحملات والرغبة في التوسع، بدأت المشكلة تظهر بوضوح: كل عميل له حالة مختلفة، كل طلب يحتاج متابعة، وكل مشروع له تفاصيل كثيرة يجب توضيحها للعميل قبل التواصل. هذا جعل المتابعة مرهقة، والصورة غير واضحة للإدارة، وصعّب معرفة حالة كل عميل أو طلب في الوقت المناسب. الشركة كانت تحتاج إلى توسيع أعمالها والوصول إلى شريحة أكبر من العملاء، لكنها كانت تواجه تحديًا مهمًا: كلما زاد حجم الطلب، احتاجت إلى توظيف مزيد من موظفي المبيعات لمجرد الرد، الشرح، المتابعة، وتسجيل البيانات.',
  ...
  },
  {
    ...
    outcome_en: 'A demo solution built around a repeated market problem: companies receive customers through websites, WhatsApp, and calls, but follow-up is scattered between employees and does not always turn into clear data or accurate decisions. We designed an AI layer that connects with the current system and communication channels to update each customer file, organize follow-up, and improve management visibility.',
    outcome_ar: 'نسخة تجريبية لحل مبني على مشكلة متكررة في السوق: شركات تستقبل العملاء من الموقع، الواتساب، والمكالمات، لكن المتابعة تتوزع بين الموظفين ولا تتحول دائمًا إلى بيانات واضحة أو قرارات دقيقة. صممنا طبقة ذكاء اصطناعي ترتبط بالنظام الحالي وقنوات التواصل لتحديث ملف كل عميل، تنظيم المتابعة، ورفع وضوح الإدارة.',
    image_url: null,
    gallery_images: [],
    demo_url: null,
    ...
    situation_en: 'Many customer-facing companies face the same problem: customers arrive through phone calls, WhatsApp, or website chat, but their data and follow-up are scattered between employees, conversations, manual notes, and spreadsheets. After every interaction, an employee usually needs to write a summary, update the customer stage, define the next follow-up date, and report updates to management. As the number of customers grows, details start to get lost: a customer is not followed up, an interaction is not summarized, an opportunity does not move to the next stage, or management sees the full picture too late. The issue is not only the number of communication channels. It is the lack of an intelligent layer that connects those channels to one follow-up system.',
    situation_ar: 'كثير من الشركات التي تعتمد على التواصل مع العملاء تعاني من نفس المشكلة: العميل قد يأتي من مكالمة، رسالة واتساب، أو تشات على الموقع، لكن بياناته ومتابعته تتوزع بين الموظفين، المحادثات، الملاحظات اليدوية، والشيتات. الموظف يحتاج بعد كل تواصل أن يكتب ملخصًا لما حدث، يحدد مرحلة العميل، يسجل موعد المتابعة، ويبلغ الإدارة بالتحديثات. ومع زيادة عدد العملاء، تبدأ التفاصيل في الضياع: عميل لم تتم متابعته، تفاعل لم يتم تلخيصه، فرصة لم تنتقل للمرحلة التالية، أو إدارة لا ترى الصورة كاملة إلا بعد فوات الوقت. المشكلة هنا ليست في وجود قنوات تواصل فقط، بل في غياب طبقة ذكية تربط هذه القنوات بنظام متابعة واحد.',
