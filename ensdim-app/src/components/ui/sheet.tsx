...
>(({ side = "right", className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-(--accent) focus:ring-offset-2">
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex items-center gap-3 px-6 py-4 border-t border-(--border) mt-auto", className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn("text-base font-semibold text-(--text-primary)", className)} {...props} />
));
