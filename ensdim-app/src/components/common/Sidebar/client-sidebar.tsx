...
const BOTTOM_NAV: NavEntry[] = [
  ...
  { href: ROUTES.CLIENT.SETTINGS,      key: "settings",      icon: Settings },
];
  ...
  return (
    <aside className={cn(
      ...
      {isCollapsed && !forceExpanded ? (
        <SimpleTooltip content="Ensdim" side={side} delayDuration={100}>
          <button
            type="button"
        ...
        <div className="flex h-14 shrink-0 items-center gap-3 border-b border-(--sidebar-border) px-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="truncate text-[13.5px] font-semibold leading-snug text-(--text-primary)">Ensdim</p>
            {user && <p className="mt-0.5 truncate text-[11px] leading-none text-(--text-muted)">{user.email}</p>}
          </div>
          {!forceExpanded && (
            <button
              type="button"
            ...
            >
              <ChevronLeft className="size-4 rtl:scale-x-[-1]" strokeWidth={2.25} />
            </button>
          )}
        </div>
      )}
      ...
      <ScrollArea className="flex-1">
        <nav className="space-y-px p-2" aria-label="Main navigation">
          {GROUPS.map((group, gi) => (
            <div key={gi}>
              {isCollapsed && gi > 0 && <div className="mx-2 my-2 h-px bg-(--sidebar-border)" />}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className={cn(
                  "group flex w-full items-center rounded-xl transition-colors duration-150",
                ...
                {!isCollapsed && (
                  <>
                    <div className="flex-1 min-w-0 text-start">
                      <p className="truncate text-[13px] font-medium leading-snug text-(--text-primary)">{user.name}</p>
                      <p className="mt-0.5 truncate text-[11px] leading-none text-(--text-muted)">{user.email}</p>
                    </div>
                    <ChevronDown className="size-3.5 shrink-0 text-(--text-muted) opacity-0 transition-opacity duration-150 group-hover:opacity-100" strokeWidth={2.5} />
                  </>
                )}
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent side="top" align={isRTL ? "end" : "start"} sideOffset={8} className="w-56">
