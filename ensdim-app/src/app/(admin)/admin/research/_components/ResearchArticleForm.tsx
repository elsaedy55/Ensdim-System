...
import { generateSlug } from "@/lib/services/research.service";
import type { UpsertResearchInput, ResearchArticle } from "@/lib/services/research.service";
import { ImagePlus, X } from "lucide-react";

interface Props {
  defaultValues?: ResearchArticle;
  onSubmit: (data: UpsertResearchInput, imageFile?: File) => void;
  isLoading: boolean;
}

export function ResearchArticleForm({ defaultValues, onSubmit, isLoading }: Props) {
  const t = useTranslations("admin.research.form");

  const [form, setForm] = React.useState<UpsertResearchInput>({
    title_en:       defaultValues?.title_en       ?? "",
    title_ar:       defaultValues?.title_ar       ?? "",
    slug:           defaultValues?.slug           ?? "",
    category_en:    defaultValues?.category_en    ?? "Research",
    category_ar:    defaultValues?.category_ar    ?? "أبحاث",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="surface p-5 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="Why businesses lose leads..."
              required
          ...
          {inp(t("fields.titleAr"), form.title_ar, (v) => setForm((f) => ({ ...f, title_ar: v })), {
            dir: "rtl",
            placeholder: "لماذا تخسر الأعمال العملاء المحتملين...",
          })}
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {inp(t("fields.categoryEn"), form.category_en, (v) => setForm((f) => ({ ...f, category_en: v })), { placeholder: "Conversion / Operations / Systems" })}
          {inp(t("fields.categoryAr"), form.category_ar, (v) => setForm((f) => ({ ...f, category_ar: v })), { dir: "rtl", placeholder: "التحويل / التشغيل / الأنظمة" })}
        </div>
      </div>
      ...
      <div className="surface p-5 space-y-4">
        ...
        {ta(t("fields.descEn"), form.description_en, (v) => setForm((f) => ({ ...f, description_en: v })), {
          rows: 2, placeholder: "Brief summary shown on the research listing page..."
        })}
        {ta(t("fields.descAr"), form.description_ar, (v) => setForm((f) => ({ ...f, description_ar: v })), {
          rows: 2, dir: "rtl", placeholder: "ملخص قصير يظهر في صفحة قائمة الأبحاث..."
        })}
      </div>
