...
function NotificationItem({ notification, onMarkRead }: { notification: NotificationRow; onMarkRead: (id: string) => void }) {
  const inner = (
    <div className={cn(
      ...
      <div className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-full", TYPE_BG[notification.type] ?? "bg-(--bg-muted)")}>
        {TYPE_ICON[notification.type] ?? <Bell className="h-4 w-4 text-(--text-muted)" />}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          {!notification.is_read && (
            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMarkRead(notification.id); }} className="text-xs text-(--accent) hover:text-(--accent-hover) transition-colors">
              Mark as read
            </button>
          )}
        </div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <PageHeader
        ...
        actions={
          unread.length > 0 ? (
            <Button variant="secondary" size="sm" onClick={() => markAllRead.mutate(undefined, { onSuccess: () => toast.success(ta("markAllRead")) })}>
        ...
        <>
          {unread.length > 0 && (
            <section className="space-y-2">
              ...
              {unread.map((n) => <NotificationItem key={n.id} notification={n} onMarkRead={(id) => markRead.mutate(id)} />)}
            </section>
          )}
          {read.length > 0 && (
            <section className="space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-(--text-muted) px-1">{t("sections.earlier")}</p>
              {read.map((n) => <NotificationItem key={n.id} notification={n} onMarkRead={(id) => markRead.mutate(id)} />)}
            </section>
          )}
        </>
      )}
